...
def distort_coordinates(point):
    ...
    coeff = (1 + D[0]*r2 + D[1]*(r2**2) + D[4]*(r2**3))/(1 + D[5]*r2 + D[6]*(r2**2) + D[7]*(r2**3))
    point_distorted = [0,0,1]
    point_distorted[0] = point[0]*coeff + 2*D[2]*point[0]*point[1] + D[3]*(r2 + 2*point[0])
    point_distorted[1] = point[1]*coeff + 2*D[3]*point[0]*point[1] + D[2]*(r2 + 2*point[1])
    point_distorted = np.asarray(point_distorted)
    return point_distorted

def handle_edges_in_bb(shape):
    '''
    ...
    for i in range(2):
        for  j in range(2):
            refPt[i][j] = max(0, refPt[i][j])
            refPt[i][j] = min(refPt[i][j], shape[(j+1)%2])
    refPt = [(refPt[0][0],refPt[0][1]), (refPt[1][0],refPt[1][1])]

def load_global_variables(obj_base_name="OOI_mesh", extension='.obj'):
    '''
    ...
    '''
    global K, D, PC, correspondances
    # Get the camera matrix distortion coefficient
    yaml_file = [os.path.join(INPUT_DIR, f).replace("\\","/") for f in os.listdir(INPUT_DIR) if f.endswith(('.yml', '.yaml'))][0]
    yaml_file = cv2.FileStorage(os.path.join(INPUT_DIR, yaml_file).replace("\\","/"), cv2.FILE_STORAGE_READ)
    ...
    with open(os.path.join(INPUT_DIR, "../output/correspondances_by_name.json").replace("\\","/"), encoding='utf-8') as json_file:
    ...
    if input_path.endswith('ply'):
        ...
        with open(input_path) as f:
            ...
            PC = np.asarray(PC_list)
    return K, D, PC, correspondances



def search_bb(image_name):
    '''
    ...
    R = np.array(correspondances[image_name]['rotation'])
    T = np.array([correspondances[image_name]['center']])
    ...
    points_in_camera_frame = np.matmul(transform, PC.T)[:3].T
    ...
    # Distort coordinates
    points_in_camera_frame_homogenous_distorted = [distort_coordinates(point) for point in points_in_camera_frame_homogenous]
    ...
    points_projected = np.matmul(K, points_in_camera_frame_homogenous_distorted.T).T

    # Compute the bounding box based on the reprojection of the point cloud
    compute_bb_with_projected_points(points_projected)

    return points_projected


def compute_bb_with_projected_points(points_projected, min_padding=10, max_padding=20):
    '''
    ...
    refPt = [[min(xs) - random.randint(min_padding, max_padding), min(ys) - random.randint(min_padding, max_padding)],[max(xs) + random.randint(min_padding, max_padding), max(ys) + random.randint(min_padding, max_padding)]]
    for i in range(2):
        for  j in range(2):
            refPt[i][j] = int(refPt[i][j])
def is_bb_yolo_correct(min_height=300, min_width=300):
    '''
        Check if the bounding box is too small to used in yolo training
    '''
    if abs(refPt[0][0]-refPt[1][0])<min_width or abs(refPt[0][1]-refPt[1][1])<min_height:
        return False
    else:
        return True
...
def create_bb(image_path, object_class="Yellow_tool"):
    '''
    ...
    with open(bb_path, 'w') as f:
        f.write("{} {} {} {} {}".format(object_class ,refPt[0][0], refPt[0][1], refPt[1][0]-refPt[0][0], refPt[1][1]-refPt[0][1]))


def do_cropping(image_path, nb_images, i, searching=False):
    '''
    ...
    if not os.path.exists(cropped_path):
        ...
        if searching:
            ...
            search_bb_naive(image2)
            create_bb(image_path)
...
def crop_images_using_bbs(bb_dir):
    '''
    ...
    for bb_file in bb_files:
        ...
        image_path = os.path.join(INPUT_DIR, image_name + '.jpg').replace("\\","/")
        if not os.path.exists(image_path):
            image_path = os.path.join(INPUT_DIR, image_name + '.png').replace("\\","/")
        with open(bb_file) as f:
            text = f.read()
            coordinates = text.split(',')
            refPt = []
            refPt.append((int(coordinates[0]), int(coordinates[1])))
            refPt.append((int(coordinates[2]), int(coordinates[3])))
        i += 1
        do_cropping(image_path, nb_images, i)


def crop_images(save_bbs=True, obj_base_name="OOI", for_yolo=False):
    '''
    ...
    if os.path.exists(os.path.join(INPUT_DIR, '../images_plus_ultra').replace("\\","/")):
        images_dir = os.path.join(INPUT_DIR, '../images_plus_ultra').replace("\\","/")

    # Load global variables
    load_global_variables(obj_base_name=obj_base_name)
    ...
    for image_name in correspondances:
        ...
        search_bb(image_name)
        if not os.path.exists(os.path.join(INPUT_DIR, image_name).replace("\\","/")):
...
def display_projection(points_projected, shape):
    '''
...
def visualize_projection():
    ...
    '''
    load_global_variables(obj_base_name="OOI_mesh")
    images_dir = INPUT_DIR
    for image_name in correspondances:
        # Load image
        image = cv2.imread(os.path.join(images_dir, image_name).replace("\\","/"))
        ...
        image_path = os.path.join(images_dir, image_name).replace("\\","/")
        print(image_name)
        display_projection(points_projected, shape)

def display_bb(clone):
    '''
        This function displays the current bounding box on the current image.
        Mostly used for debugging purpose.
        What to do when keys are pressed needs to be manually specified in the code.
...
def create_obj_file_from_array(points, file_base_name='Test'):
    '''
    ...
    with open(os.path.join(INPUT_DIR, '../output/'+ file_base_name + '.obj').replace("\\","/"), 'w') as f:
        f.write(txt)
...
if __name__ == '__main__':
    ...
    crop_images(obj_base_name="OOI_PC")
