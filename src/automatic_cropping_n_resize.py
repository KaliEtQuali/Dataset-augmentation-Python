...
class automatic_cropator:
    def __init__(self, INPUT_DIR_=INPUT_DIR):
        self.refPt = []
        self.cropping = False
        self.K = []
        self.PC = []
        ...
        self.OOI_DIR = os.path.join(self.INPUT_DIR,"../output").replace("\\","/")
        self.yml_dir=os.path.join(self.INPUT_DIR,"..").replace("\\","/")
        self.json_path=os.path.join(self.INPUT_DIR,"../output/correspondances_by_name.json").replace("\\","/")
    ...
    def load_variables(self, obj_base_name="OOI_PC"):
        '''
        ...
        yaml_file = [os.path.join(self.yml_dir, f).replace("\\","/") for f in os.listdir(self.yml_dir) if f.endswith(('.yml', '.yaml'))][0]
        yaml_file = cv2.FileStorage(os.path.join(self.yml_dir, yaml_file).replace("\\","/"), cv2.FILE_STORAGE_READ)
        ...
        with open(os.path.join(self.json_path).replace("\\","/"), encoding='utf-8') as json_file:
            self.correspondances = json.load(json_file)

        # Get the coordinates of the pointcloud of the tool
        input_path = os.path.join(self.OOI_DIR, obj_base_name + '.obj').replace("\\","/")
        if not os.path.exists(input_path):
            input_path = os.path.join(self.OOI_DIR, obj_base_name + '.ply').replace("\\","/")
    ...
    def search_bb(self, image_name, compute_bb=True):
        '''
        ...
        points_in_camera_frame_homogenous = [[point[0]/point[2], point[1]/point[2], point[2]/point[2]] for point in points_in_camera_frame if point[2]!=0]
    ...
    def crop_images(self, save_bbs=True, show_bb=False):
        '''
...
def perform_automatic_cropping(obj_base_name="OOI_PC"):
    ...
    Croper.load_variables(obj_base_name="OOI_PC")
    Croper.crop_images()
