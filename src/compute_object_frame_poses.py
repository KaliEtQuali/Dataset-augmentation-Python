...
POSES_DIR = os.path.join(INPUT_DIR, '../output')
CORRESPONDANCES_JSON_PATH = os.path.join(INPUT_DIR, "../output/", 'correspondances_by_name.json')
...
def compute_new_axis(origin, up_point, forward_point):
    '''
        This function computes the X, Y and Z axiz defined by the given origin point, upward point and forward point
            ...
            The direction origin ->  forward is assumed to be the X axis
...
def transform_poses(T):
    ...
    for image in correspondances:
        ...
        '''T_to_camera_frame = compute_tranfomation_mat(np.asarray([correspondances[image]['center']]),
            np.asarray([0,0,1]).transpose(),
            np.asarray([1,0,0]).transpose()
            )'''
...
def apply_transformation_to_pcd(pcd_path, T, output_dir, output_name):
    ...
    '''with open(os.path.join(output_dir, "testo.obj"), 'w') as pcd_transformed_file:
        for point in pointcloud_transformed:
            pcd_transformed_file.write("{} {} {} {}\n".format("v", point[0], point[1], point[2]))
    print('obj file saved at: ', os.path.join(output_dir, "testo.obj"))'''
...
def run():
    ...
    create_camera_frame_pcd_files(POSES_DIR, os.path.join(POSES_DIR, 'pointcloud.pcd'))
