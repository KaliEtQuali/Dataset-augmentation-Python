...
def create_correspondances_json_from_aruco_files():
    input_dir = 'C:/Users/owner/Desktop/K/ToolWithMarkers'
    ...
    images = os.listdir(os.path.join(input_dir, 'images_localized'))
    for image in images:
...
def goloum():
    input_dir = INPUT_DIR
    out_dir = os.path.join(INPUT_DIR, '../test_images')
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
    with open("/media/jrl/UBUNTU16/K/test_names.txt") as f:
        names_raw = f.read()
        names = names_raw.splitlines()
    for namel in names:
        name = namel.split('.')[0]+'.jpg'
        image_path = os.path.join(input_dir, name)
        dest_path = os.path.join(out_dir, name)
        copyfile(image_path, dest_path)

if __name__ == '__main__':
    print(os.path.join(INPUT_DIR, '..', 'yolo.txt'))
